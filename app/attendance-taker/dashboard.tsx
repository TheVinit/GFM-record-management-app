...
export default function AttendanceTakerDashboard() {
  ...
  const [sessionRecords, setSessionRecords] = useState<any[]>([]);
    ...
    useEffect(() => {
      if (yearFilter && deptFilter) {
        checkCompletedDivisions();
      }
    }, [yearFilter, deptFilter, viewMode]);
        ...
        <View style={styles.lastSubmittedCard}>
          ...
          <View style={styles.lastSubmittedDetails}>
            <Text style={styles.lastSubmittedText}>
              Class: <Text style={{ fontWeight: 'bold' }}>{lastSubmitted.session.department} {lastSubmitted.session.class} ({lastSubmitted.session.division})</Text>
            </Text>
            <Text style={styles.lastSubmittedText}>
            ...
            <View style={styles.sessionDetails}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.sessionInfoTitle}>
                        {selectedSession.department} - {selectedSession.class} ({selectedSession.division})
                      </Text>
              ...
              <View style={styles.recordsTable}>
                <View style={styles.tableHeader}>
                  <Text style={[styles.tableHead, { flex: 1 }]}>Roll</Text>
                ...
                {sessionRecords.map((rec) => (
                  <View key={rec.id} style={styles.tableRow}>
                    <Text style={[styles.tableCell, { flex: 1 }]}>{rec.studentPrn.slice(-3)}</Text>
                    <Text style={[styles.tableCell, { flex: 3 }]}>{rec.fullName}</Text>
                    <View style={[
                      styles.statusBadge, 
                    ...
                    >
                      <View style={styles.sessionItemContent}>
                        <Text style={styles.sessionItemTitle}>
                          {session.department} - {session.class} ({session.division})
                        </Text>
